...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto">
      ...
      {/* Overview Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8 animate-fade-in animate-fade-in-delay-1">
        {[
          { label: 'Total Students', value: o?.totalStudents ?? '--', icon: '🎓', sub: `${o?.verifiedStudents ?? 0} verified`, color: 'rgba(99,102,241,0.1)', border: 'rgba(99,102,241,0.15)' },
          { label: 'Total Quizzes', value: o?.totalQuizzes ?? '--', icon: '📝', sub: `${o?.totalAttempts ?? 0} attempts`, color: 'rgba(16,185,129,0.1)', border: 'rgba(16,185,129,0.15)' },
          { label: 'References', value: o?.totalReferences ?? '--', icon: '📚', sub: 'shared resources', color: 'rgba(34,211,238,0.1)', border: 'rgba(34,211,238,0.15)' },
          { label: 'Platform Avg', value: o ? `${o.platformAvgScore}%` : '--', icon: '⭐', sub: `${o?.recentAttempts ?? 0} this week`, color: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.15)' },
        ].map(card => (
          <div key={card.label} className="glass-card-hover p-5">
            ...
            {loadingAI ? <div className="skeleton" style={{ width: '60%', height: 24 }} /> : (
              <>
                <span className="text-2xl font-bold text-text-1 block">{card.value}</span>
      ...
      {/* Main Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8 animate-fade-in animate-fade-in-delay-2">
        {/* Level Distribution */}
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-text-1 mb-4 flex items-center gap-2">
            🧠 Student Levels <span className="badge badge-primary" style={{ fontSize: 9 }}>ANN</span>
          </h3>
          {loadingAI ? (
            <div className="space-y-4">{[1,2,3,4].map(i => <div key={i} className="skeleton" style={{ height: 32 }} />)}</div>
          ) : analytics?.levelDistribution ? (
            <div className="space-y-4">
              {Object.entries(analytics.levelDistribution).map(([level, count]) => {
                const total = Object.values(analytics.levelDistribution).reduce((s, v) => s + v, 0);
                const pct = total > 0 ? Math.round((count / total) * 100) : 0;
                return (
                  <div key={level}>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-sm text-text-2 flex items-center gap-1.5">{levelEmoji[level]} {level}</span>
                      <span className="text-sm font-semibold text-text-1 font-mono">{count} <span className="text-text-3 font-normal">({pct}%)</span></span>
                    </div>
                    <div className="progress-bar" style={{ height: 8 }}>
                      <div className="progress-fill" style={{ width: `${pct}%`, background: levelColors[level], minWidth: count > 0 ? 8 : 0 }} />
                    </div>
                  </div>
                );
              })}
              <p className="text-xs text-text-3 mt-3 pt-3 border-t border-glass-border">
                ANN classification based on score, consistency, efficiency
              </p>
            </div>
          ) : <p className="text-text-3 text-sm text-center py-6">No data yet</p>}
        </div>
        ...
        <div className="glass-card p-6">
          ...
          {loadingAI ? (
            <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 48 }} />)}</div>
          ) : (analytics?.subjectPerformance?.length || 0) > 0 ? (
                ...
                <div key={sp.subject} className="p-3 rounded-xl" style={{ background: 'var(--glass)' }}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-sm font-bold font-mono" style={{
                      color: sp.avgScore >= 70 ? 'var(--success)' : sp.avgScore >= 50 ? 'var(--warning)' : 'var(--danger)'
                    }}>{sp.avgScore}%</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="progress-bar flex-1">
                      <div className="progress-fill" style={{
                        width: `${sp.avgScore}%`,
                        background: sp.avgScore >= 70 ? 'var(--success)' : sp.avgScore >= 50 ? 'linear-gradient(90deg, var(--warning), var(--success))' : 'linear-gradient(90deg, var(--danger), var(--warning))',
                      }} />
                    </div>
                    <span className="text-xs text-text-3 flex-shrink-0 font-mono">{sp.totalAttempts}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="glass-card p-6">
          ...
          {loadingAI ? (
            <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 52 }} />)}</div>
          ) : (analytics?.topStudents?.length || 0) > 0 ? (
              ...
              {analytics!.topStudents.slice(0, 6).map((s, i) => (
                <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--glass)' }}>
                  ...
                  <div className="text-right flex-shrink-0">
                    <span className="text-sm font-bold font-mono" style={{
                      color: s.avgScore >= 70 ? 'var(--success)' : s.avgScore >= 50 ? 'var(--warning)' : 'var(--danger)'
                    }}>{s.avgScore}%</span>
      ...
      {/* AI Models Footer */}
      <div className="mt-5 glass-card p-4 animate-fade-in animate-fade-in-delay-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <span className="text-xs text-text-3">🧠 Analytics powered by</span>
          <div className="flex gap-2 flex-wrap">
            {['ANN Classifier', 'Autoencoder', 'LSTM', 'NCF', 'NLP'].map(m => (
              <span key={m} className="badge badge-primary" style={{ fontSize: 9 }}>{m}</span>
            ))}
          </div>
        </div>
      </div>
