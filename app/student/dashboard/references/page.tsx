    ...
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto">
      <div className="page-header animate-fade-in flex items-start justify-between">
        <div>
          ...
          <p className="text-text-3 text-sm mt-1">Share and discover study resources</p>
        </div>
      ...
      {/* Search & Sort */}
      <div className="flex gap-3 mb-6 animate-fade-in animate-fade-in-delay-1">
      ...
      {loading ? (
        ...
        <div className="glass-card empty-state">
          ...
          <p className="text-text-2 text-lg font-medium mb-1">No references found</p>
          <p className="text-text-3 text-sm">Be the first to share a resource!</p>
        </div>
      ) : (
          ...
          {refs.map(ref => (
            <div key={ref.id} className="glass-card-hover p-5 flex justify-between items-start gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-text-3 text-xs mt-2">By {ref.posted_by_name} ({ref.posted_by_role === 'student' ? '🎓 Student' : '🛡️ Admin'}) · {new Date(ref.created_at).toLocaleDateString()}</p>
              </div>
